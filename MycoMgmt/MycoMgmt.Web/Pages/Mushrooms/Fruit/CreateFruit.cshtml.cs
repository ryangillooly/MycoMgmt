...
public class FruitModel : PageModel
{
    ...
    private const string url = "http://localhost:6002/fruit";
    public string responseString;
    public List<string> strains;
    
    public FruitModel(IHttpClientFactory clientFactory)
    {
    ...
    public async Task OnPostAsync()
    {
        ...
        string? wetWeight    = Request.Form["wetWeight"];
        string? dryWeight    = Request.Form["dryWeight"];
        ...
        string? harvestedOn = Request.Form["harvestedOn"];
        string? harvestedBy = Request.Form["harvestedBy"];
        ...
        if (wetWeight    is not null and not "") paramString += $"wetWeight={wetWeight}&";
        if (dryWeight    is not null and not "") paramString += $"dryWeight={dryWeight}&";
        ...
        if (harvestedOn  is not null and not "") paramString += $"harvestedOn={harvestedOn}&";
        if (harvestedBy  is not null and not "") paramString += $"harvestedBy={harvestedBy}&";
